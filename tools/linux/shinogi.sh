...
cmd_screenshot() {
    out="${1:-$HOME_DIR/logs/screen.png}"
    case "$out" in
        /*) ;;
        *) out="$PWD/$out" ;;
    esac
...
# GEM is a keyboard-and-mouse system with no other way in, so an agent
...
cmd_mouse() {
    case "${1:-}" in
        click)
            ...
            ;;
        move)
            [ $# -eq 3 ] || die "usage: shinogi mouse move <x> <y>"
            mon mouse_move "$2" "$3" >/dev/null
            ;;
        *) die "usage: shinogi mouse move <x> <y> | shinogi mouse click" ;;
    esac
}
...
SHINOGI_LOADER (auto|native|bundled).
EOF
}
